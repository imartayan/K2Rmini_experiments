...
use helicase::*;
use sbwt::{
    BitPackedKmerSortingMem, LcsArray, SbwtIndex, SbwtIndexBuilder, StreamingIndex, SubsetMatrix,
};
...
fn main() -> io::Result<()> {
    ...
    let start = Instant::now();
    process_query_streaming(&args, Arc::clone(&sbwt), Arc::clone(&lcs))?;
    eprintln!(
        "Took {:.02} s, RAM: {:.03} GB",
        start.elapsed().as_secs_f64(),
        mem_usage_gb()
...
fn process_query_streaming(
    ...
    for _ in 0..num_consumers {
        ...
        let sbwt_clone = Arc::clone(&sbwt);
        let lcs_clone = Arc::clone(&lcs);

        let handle = thread::spawn(move || {
            let index = StreamingIndex::new(&*sbwt_clone, &lcs_clone);
            while let Ok((ids, seqs, ends)) = record_rx_clone.recv() {
                if ends.len() == 1 {
                    ...
                    for (len, _) in index.matching_statistics(seq) {
                        kmer_match_count += len.saturating_sub(kmer_size - 1);
